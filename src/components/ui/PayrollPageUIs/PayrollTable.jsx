...
import PayslipTemplate from "./PayslipTemplate"; // <--- 1. IMPORT THIS
...
const PayrollTable = () => {
  ...
  const handleViewPayslip = (record) => {
    // We need to format the data to match what PayslipTemplate expects
    const formattedData = {
      payrollRun: {
      ...
      employee: {
        ...
        department: "LJA Power", // You can fetch this if available
        tin: record.email, // Using email as placeholder if TIN missing
        sss: "-",
        philhealth: "-",
        pagibig: "-"
      },
      earnings: [
        { label: "Basic Salary", amount: record.basic_salary, units: "Attendance" },
        { label: "Overtime", amount: record.overtime_pay, units: "Approved OT" },
        { label: "Allowances", amount: record.allowances },
      ],
      // IMPORTANT: Mapping the JSON details we saved in the backend!
      deductions: record.details.deduction_breakdown.map(d => ({
        label: d.name,
        amount: d.amount
      })),
      // If we have separate loans in the future, filter them here. 
      // For now, they are all in deduction_breakdown.
      loans: [], 
      totals: {
        gross: parseFloat(record.basic_salary) + parseFloat(record.overtime_pay) + parseFloat(record.allowances),
        ...
        <div className="overflow-x-auto flex-1">
          <table className="table table-sm w-full">
            ...
            <tbody className="text-sm divide-y divide-white/5">
              {isFetchingDetails ? (
                <tr><td colSpan="6" className="h-64 text-center"><Loader2 className="animate-spin size-8 text-primary mx-auto" /></td></tr>
              ) : filteredRecords.length === 0 ? (
                <tr><td colSpan="6" className="h-64 text-center opacity-40"><User size={32} className="mx-auto"/> No employees found.</td></tr>
              ) : (
                filteredRecords.map((rec) => (
                  <tr key={rec.id} className="hover:bg-base-200/40 transition-colors group">
                    {/* ... (Keep your existing TD columns 1-5 here) ... */}
                     <td className="pl-6 py-3">
                      <div className="flex items-center gap-3">
                        <div className="avatar placeholder">
                          <div className="bg-neutral text-neutral-content rounded-full w-8">
                            <span className="text-xs">{rec.fullname.charAt(0)}</span>
                          </div>
                        </div>
                        <div>
                    ...
                    <td className="text-right font-medium opacity-70 text-xxs">{formatMoney(rec.basic_salary)}</td>
                    <td className="text-right font-medium text-success text-xxs">{parseFloat(rec.overtime_pay) > 0 ? `+${formatMoney(rec.overtime_pay)}` : "-"}</td>
                    <td className="text-right font-medium text-error">{parseFloat(rec.deductions) > 0 ? `-${formatMoney(rec.deductions)}` : "-"}</td>
                    <td className="text-right"><div className="font-bold text-base text-base-content text-xxs">{formatMoney(rec.net_pay)}</div></td>
